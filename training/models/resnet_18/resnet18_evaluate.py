"""
resnet18_evaluate.py
────────────────────
Loads the saved ResNet-18 checkpoint, runs inference on VAL and TEST splits,
and generates the following plots saved to resnet_18/plots/:
...
Run from project root:
    python -m training.models.resnet_18.resnet18_evaluate
"""
...
CKPT_PATH   = os.path.join(BASE_DIR, "resnet18_model.pth")
LOG_PATH    = os.path.join(BASE_DIR, "resnet18_epoch_log.csv")
PLOT_DIR    = os.path.join(BASE_DIR, "plots")
os.makedirs(PLOT_DIR, exist_ok=True)

BATCH_SIZE  = 8
PRETRAINED  = True          # must match what was used during training
...
def run_inference(split: int, device):
    ds     = ResNetEchoDataset(split=split, augment=False)
    loader = DataLoader(ds, batch_size=BATCH_SIZE, shuffle=False, num_workers=0)

    model = ResNet18EFModel(pretrained=True).to(device)    # must match training
...
def plot_training_curves():
    ...
    ax1.axvline(best_epoch, color="gray", lw=1, ls=":",
