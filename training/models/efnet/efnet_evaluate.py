...
efnet_evaluate.py
─────────────────
Loads the saved EF-Net checkpoint, runs inference on VAL and TEST splits,
and generates all comparison plots saved to efnet/plots/.
  ...
  4. error_by_ef_range.png     — MAE by clinical EF category
  5. training_curves.png       — train/val loss + MAE over epochs

Run from project root:
    python -m training.models.efnet.efnet_evaluate
"""
...
from training.models.efnet.efnet_dataset_loader import EFNetEchoDataset, NUM_FRAMES
from training.models.efnet.efnet_model import EFNet

# ── Paths ──────────────────────────────────────────────────────────────────
BASE_DIR    = os.path.dirname(__file__)
CKPT_PATH   = os.path.join(BASE_DIR, "efnet_model.pth")
LOG_PATH    = os.path.join(BASE_DIR, "efnet_epoch_log.csv")
...
def run_inference(split: int, device):
    ds     = EFNetEchoDataset(split=split, sample_mode="uniform")
    loader = DataLoader(ds, batch_size=BATCH_SIZE, shuffle=False, num_workers=0)

    model = EFNet(num_frames=NUM_FRAMES, dropout_rate=0.4, num_heads=4).to(device)
