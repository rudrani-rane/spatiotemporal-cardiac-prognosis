"""
efficientnet_b4_evaluate.py
────────────────────────────
Loads the saved EfficientNet-B4 checkpoint, runs inference on VAL and TEST
splits, and generates all comparison plots saved to efficientnet_b4/plots/.

  1. true_vs_predicted.png
  2. residuals.png
  3. bland_altman.png
  4. error_by_ef_range.png
  5. training_curves.png

Run from project root:
    python -m training.models.efficientnet_b4.efficientnet_b4_evaluate
"""
...
CKPT_PATH   = os.path.join(BASE_DIR, "efficientnet_b4_model.pth")
LOG_PATH    = os.path.join(BASE_DIR, "efficientnet_b4_epoch_log.csv")
...
# EfficientNet-B4 uses a distinct teal colour to differentiate from
# CNN (blue), ResNet-18 (orange), EF-Net (green) in combined reports
C_SCATTER = "#7B2FBE"
C_LINE    = "#C55A11"
C_HIST    = "#7B2FBE"
C_BAR     = "#7B2FBE"
...
def run_inference(split: int, device):
    ds     = EfficientNetEchoDataset(split=split, augment=False)
    loader = DataLoader(ds, batch_size=BATCH_SIZE, shuffle=False, num_workers=0)

    # pretrained=True loads architecture — weights loaded from checkpoint next
    model = EfficientNetB4EFModel(pretrained=True, dropout_rate=0.4).to(device)
...
def plot_true_vs_pred(true, pred, split_name):
    ...
    ax.plot([lo, hi], [lo, hi], color=C_LINE, lw=1.8, ls="--",
    ...
    ax.set_aspect("equal")
    _style_ax(ax, f"{MODEL_LABEL} — True vs Predicted LVEF ({split_name})",
...
def plot_residuals(true, pred, split_name):
    ...
    _style_ax(ax, f"{MODEL_LABEL} — Prediction Residuals ({split_name})",
...
def plot_bland_altman(true, pred, split_name):
    ...
    ax.legend(fontsize=9)
    _style_ax(ax, f"{MODEL_LABEL} — Bland-Altman Plot ({split_name})",
...
def plot_error_by_ef_range(true, pred, split_name):
    ...
    for lo, hi in zip(EF_BINS[:-1], EF_BINS[1:]):
        ...
        mae_per_bin.append(
            np.mean(np.abs(true[mask]-pred[mask])) if mask.sum() > 0 else 0)
    ...
    ax.legend(fontsize=9)
    _style_ax(ax, f"{MODEL_LABEL} — MAE by Clinical EF Category ({split_name})",
...
def plot_training_curves():
    if not os.path.exists(LOG_PATH):
        print(f"  [SKIP] No log at {LOG_PATH}")
        return
    ...
    best_e = epochs[int(np.argmin(vl_loss))]
    ax1.axvline(best_e, color="gray", lw=1, ls=":", label=f"Best ({best_e})")
    ...
    best_mae_e = epochs[int(np.argmin(vl_mae))]
    ax2.axvline(best_mae_e, color="gray", lw=1, ls=":", label=f"Best ({best_mae_e})")
